...
CircuitPython library for TCA9534 Low Voltage 8-Bit I2C
and SMBUS Low-Power I/O Expander with Interrupt Output
...
class TCA9534:
    ...
    def read_bit(self, register_address, gpio_position):
        ...
        return (
            self.read_register(register_address) & (1 << gpio_position)
        ) >> gpio_position
    ...
    def write_bit(self, register_address, gpio_position, bit_value):
        ...
        byte_value = (byte_value & ~bit_mask) | (
            (bit_value << gpio_position) & bit_mask
        )
    ...
    def set_gpio_mode(self, gpio_position, gpio_mode):
        ...
        port_configuration = (port_configuration & ~gpio_mask) | (
            (gpio_mode << gpio_position) & gpio_mask
        )
    ...
    def set_gpio_invert(self, gpio_position, inversion_mode):
        ...
        port_inversion = self.read_register(TCA9534_REGISTER_INVERSION)
        gpio_mask = 1 << gpio_position
        port_inversion = (port_inversion & ~gpio_mask) | (
            (inversion_mode << gpio_position) & gpio_mask
        )
    ...
    def set_gpio(self, gpio_position, gpio_out_status):
        ...
        gpio_mask = 1 << gpio_position
        port_out_status = (port_out_status & ~gpio_mask) | (
            (gpio_out_status << gpio_position) & gpio_mask
        )
